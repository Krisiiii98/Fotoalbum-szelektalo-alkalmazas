...
#képek méretezése a canvas méretéhez(méretarányosan)
...
def fault_detection(images):
    ...
    #Over- and Underexposure detection
    image_faults.exposed, image_features.quality = estimate_exposure(img)
    ...
    #purple_fringing detection
    image_faults.purple, image_typ.purple_img  = purple_fringe_detection(img)
...
def faultimage_correction(images):
    ...
    #new_img_view['state'] = tk.NORMAL
    image_switch.configure(state=NORMAL)
    
    image_typ.repaired_img = img
    cv2.imwrite('new_img.jpg', image_typ.repaired_img)
...
#classification = tk.Button(gui,text='Osztályozás',bg='#364156', foreground='white', font=('arial', 10, 'bold'))
#classification.place(x=1170,y=430)
...
#new_img_view = tk.Button(gui,text='Megtekintés', command=lambda: newimg_displaying(), bg='#364156', foreground='white', font=('arial', 10, 'bold'), relief="raised")
#new_img_view.place(x=875,y=530)
